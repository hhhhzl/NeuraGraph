...
-------------------------------------------------
   ...
   Description : hugegraph clients, integrated apis for hugegraph server
   Author :       hhhhzl
   date:          2023/12/19
---------------------------------------------------
----------------------------------------------------
接口说明：
    get_all_graphs: 列出数据库中全部的图
    get_version: 查看HugeGraph的版本信息
    get_graph_info: 查看某个图的信息
    clear_graph_alldata: 清空某个图的全部数据，
        包括schema、vertex、edge和索引等，
        该操作需要管理员权限
    get_graph_config: 查看某个图的配置，该操作需要管理员权限
    create_property_key: 创建一个property_key
    add_property_key_userdata: 为已存在的 PropertyKey 添加userdata
    delete_property_key_userdata: 为已存在的 PropertyKey 移除 userdata
    get_graph_all_property_keys：获取所有的 PropertyKey
    get_graph_property_key_by_name: 根据name获取PropertyKey
    delete_graph_property_key_by_name: 根据name删除PropertyKey
    create_vertex_label: 创建一个VertexLabel
    add_vertex_label_properties: 为一个VertexLabel添加properties属性
    delete_vertex_label_properties: 为一个VertexLabel移除properties属性(暂未支持)
    add_vertex_label_userdata: 为一个VertexLabel添加userdata属性
    delete_vertex_label_userdata: 为一个VertexLabel删除userdata属性
    get_all_vertex_labels(self): 获取所有的VertexLabel
    get_vertex_label_by_name: 根据name获取VertexLabel
    delete_vertex_label_by_name: 根据name删除VertexLabel
    create_edge_label: 创建一个EdgeLabel
    add_edge_label_properties: 为一个EdgeLabel添加properties
    add_edge_label_userdata: 为一个EdgeLabel添加userdata
    delete_edge_label_userdata: 为一个EdgeLabel删除userdata
    get_all_edge_labels: 获取所有的EdgeLabels
    get_edge_label_by_name: 根据name获取EdgeLabel
    delete_edge_label_by_name: 根据name删除EdgeLabel
    create_index_label: 创建一个IndexLabel
    get_all_index_labels: 获取所有的indexlabel
    get_indexLabel_by_name: 根据name获取indexlabel
    delete_index_label_by_name: 根据name删除indexlabel
    create_vertex: 创建一个顶点
    create_multi_vertex: 创建多个顶点
    update_vertex_properties: 更新顶点属性
    delete_vertex_properties: 删除顶点属性
    get_vertex_by_condition: 获取符合条件的顶点
    get_vertex_by_page:
        获取符合条件的顶点 ，
        如果提供page参数，必须提供limit参数，不允许带其他参数
    get_vertex_by_id: 根据ID 获取顶点
    delete_vertex_by_id:根据ID 删除顶点
    create_edge: 创建一条边
    create_multi_edge: 创建多条边
    update_edge_properties: 更新边的属性
    delete_edge_properties: 删除边的属性
    get_edge_by_condition: 根据条件查询获取边
    get_edge_by_page:
        获取符合条件的边, 如果提供page参数, 必须提供limit参数, 不允许带其他参数
    get_edge_by_id: 根据Id 获取边
    delete_edge_by_id: 根据Id 删除边
    traverser_shortest_path:
        根据起始顶点、目的顶点、方向、边的类型（可选）和最大深度，查找一条最短路径
    traverser_kout:
        根据起始顶点、方向、边的类型（可选）和深度depth，
        查找从起始顶点出发恰好depth步可达的顶点
    traverser_kneighbor:
        根据起始顶点、方向、边的类型（可选）和深度depth，
        查找包括起始顶点在内、depth步之内可达的所有顶点
    traverser_vertices: 根据顶点的id列表，批量查询顶点
    create_variables: 创建某个键值对
    update_variables: 更新某个键值对
    get_all_variables: 列出全部键值对
    get_variables_by_key: 列出某个键值对
    delete_variables: 删除某个键值对
    get_graph_all_tasks: 列出某个图中全部的异步任务
    get_graph_task_info: 查看某个任务的信息
    delete_graph_task_info: 删除某个异步任务信息，不删除异步任务本身
    rebuild_index_label: 重建IndexLabel
    rebuild_vertex_label: 重建vertexlabel
    rebuild_edge_label: 重建edgelabel
    execute_germlin_get
        向HugeGraphServer发送gremlin语句（GET），同步执行
    execute_germlin_post
        向HugeGraphServer发送gremlin语句（post），同步执行
    execute_germlin_postjob:
        向HugeGraphServer发送gremlin语句（post），异步执行
        异步执行Gremlin语句暂不支持aliases，可以使用 graph 代表要操作的图，
        也可以直接使用图的名字， 例如 hugegraph;
...
class HugeGraphClient(object):
    ...
    def add_property_key_userdata(self, property_name, user_data):
        """
    ...
    def delete_property_key_userdata(self, property_name, user_data):
        """
    ...
    def get_graph_all_property_keys(self):
        """
        获取所有的 PropertyKey
        :return:
        """
    ...
    def get_graph_property_key_by_name(self, property_name):
        """
        根据name获取PropertyKey
        :param property_name:
        :return:
    ...
    def delete_graph_property_key_by_name(self, property_name):
        """
        根据name删除PropertyKey
        :param property_name:
        :return:
    ...
    def add_vertex_label_properties(self, name, properties, nullable=''):
        """
    ...
    # def delete_vertex_label_properties(self,name, properties):
    #     """
    #     ***(此功能暂不支持)***
    ...
    def add_vertex_label_userdata(self, name, userdata):
        """
    ...
    def delete_vertex_label_userdata(self, name, userdata):
        """
    ...
    def get_vertex_label_by_name(self, name):
        """
        根据name获取VertexLabel
        :param name:
        :return:
    ...
    def delete_vertex_label_by_name(self, name):
        """
        根据name删除VertexLabel
        :param name:
        :return:
    ...
    def create_edge_label(self, data):
        """
        创建一个EdgeLabel
    ...
    def add_edge_label_properties(self, name, properties, nullable=''):
        """
    ...
    def add_edge_label_userdata(self, name, userdata):
        """
    ...
    def delete_edge_label_userdata(self, name, userdata):
        """
    ...
    def get_edge_label_by_name(self, name):
        """
        根据name获取EdgeLabel
        :param name:
        :return:
    ...
    def delete_edge_label_by_name(self, name):
        """
        根据name删除EdgeLabel
        :param name:
        :return:
    ...
    def create_index_label(self, data):
        """
        创建一个IndexLabel
    ...
    def get_index_label_by_name(self, name):
        """
        根据name获取indexlabel
        :param name:
        :return:
    ...
    def delete_index_label_by_name(self, name):
        """
        根据name删除indexlabel
        :param name:
        :return:
    ...
    def get_graph_task_info(self, task_id):
        """
        查看某个任务的信息
        :param task_id:
        :return:
    ...
    def delete_graph_task_info(self, task_id):
        """
    ...
    def rebuild_index_label(self, indexlabel_name):
        """
        重建IndexLabel
    ...
    def rebuild_vertex_label(self, vertexlabel_name):
        """
        重建vertexlabel
    ...
    def rebuild_edge_label(self, edgelabel_name):
        """
        重建edgelabel
